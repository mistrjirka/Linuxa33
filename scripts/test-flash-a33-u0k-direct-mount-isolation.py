#!/usr/bin/env python3
from __future__ import annotations

import importlib.util
from pathlib import Path
import sys

HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(HERE / "lib"))

spec = importlib.util.spec_from_file_location(
    "u0k_flash", HERE / "flash-a33-u0k-direct-mount-isolation.py"
)
assert spec and spec.loader
module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = module
spec.loader.exec_module(module)

module.PROFILE.validate()
assert module.PROFILE.operation == "flash-exact-u0k-direct-mount-isolation"
assert module.PROFILE.report_name == "a33-first-rootfs-u0k-direct-mount-isolation-flash.txt"
assert module.PROFILE.remote_candidate == "/tmp/a33x-u0k-direct-mount-isolation-recovery.img"

manifest = module.manifest_contract()
assert manifest["candidate"] == "U0k-direct-mount-isolation"
assert manifest["functional_base"] == "U0j-root-api-compatible"
assert manifest["cpio_payload_delta"] == "init_2nd.sh"
assert manifest["rootfs_check_skipped"] == "yes"
assert manifest["rootfs_partition_resize_skipped"] == "yes"
assert manifest["rootfs_filesystem_resize_skipped"] == "yes"
assert manifest["mount_root_partition_retained"] == "yes"
assert manifest["switch_root_retained"] == "yes"
assert manifest["skipped_second_stage_calls"] == module.SKIPPED_CALLS
assert manifest["stage_markers"] == module.STAGE_MARKERS

patch = module.patch_contract()
assert patch["operation"] == "python-byte-preserving-direct-mount-isolation"
assert patch["patch_status"] == "passed"
assert patch["shell_text_outside_two_exact_blocks_preserved"] == "yes"

assert module.sha_bytes(b"fixture") == "f16d05ec6b29248d2c61adb1e9263f78e4f7bace1b955014a2d17872cfe4064d"

try:
    module.FlashProfile(
        operation="bad operation",
        report_name="../bad.txt",
        remote_candidate="/data/bad.img",
        success_label="bad\nlabel",
    ).validate()
except ValueError:
    pass
else:
    raise AssertionError("unsafe U0k flash profile was accepted")

class DummyCommon:
    RECOVERY = "/dev/block/by-name/recovery"

    @staticmethod
    def sha_file(path: Path) -> str:
        return {
            Path("/tmp/deploy.txt"): "d" * 64,
            Path("/tmp/manifest.txt"): "m" * 64,
        }[path]


local = {
    "deploy_path": Path("/tmp/deploy.txt"),
    "root_uuid": "7b056328-bdfb-496b-ac38-2624c43c863a",
    "critical_manifest_sha": "c" * 64,
    "manifest_path": Path("/tmp/manifest.txt"),
    "candidate_size": 100663296,
    "candidate_sha": "9" * 64,
}
pairs = dict(
    module.execute_flash.__globals__["report_pairs"](
        DummyCommon,
        module.PROFILE,
        local,
        Path("/tmp/candidate.img"),
        "9" * 64,
        "2026-08-04T17:00:00+02:00",
    )
)
assert pairs["operation"] == module.PROFILE.operation
assert pairs["candidate_sha256"] == "9" * 64
assert pairs["recovery_partition_sha256"] == "9" * 64
assert pairs["userdata_written"] == "no"
assert pairs["cache_written"] == "no"
assert pairs["super_written"] == "no"
assert pairs["boot_written"] == "no"
assert pairs["recovery_written"] == "yes"
assert pairs["reboot_performed"] == "no"

print("u0k_flash_profile_self_test=passed")
print("u0k_manifest_contract=passed")
print("u0k_patch_contract=passed")
print("u0k_payload_hash_helper=passed")
print("unsafe_profile_refusal=passed")
print("shared_flash_report_contract=passed")
