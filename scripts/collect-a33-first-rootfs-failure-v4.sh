...
SUPERBLOCK="$OUT/ext4-superblock-state-v4.txt"
ROOT_STATE="$OUT/rootfs-openrc-ssh-state-v4.txt"
FOCUSED="$OUT/first-rootfs-focused-last-kmsg-v4.txt"
SUMMARY="$OUT/first-rootfs-failure-v4-summary.txt"
...
expanded = "yes" if filesystem_bytes > image_bytes else "no"
last_mounted_value = last_mounted or "empty"
...
print(f"filesystem_expanded_beyond_image={expanded}")
...
print(f"last_mounted_path={last_mounted_value}")
...
set -u
target="$1"
mountpoint=/tmp/a33x-first-rootfs-v4-check
mounted_here=no
cleanup() {
...

{
    ...
    echo "operation=collect-first-rootfs-ssh-failure-v4"
...
echo
echo "First-rootfs SSH failure evidence v4 collected."
echo "Directory: $OUT"
