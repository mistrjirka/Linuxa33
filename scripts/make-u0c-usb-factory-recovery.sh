...
for command in gzip cpio sha256sum modinfo cmp; do
...
if ! grep -q '/pdic_notifier_module\.ko$' "$entries"; then
    echo "REFUSING U0c: pdic_notifier_module.ko is absent" >&2
    exit 1
fi

pdic_entry="$(grep '/pdic_notifier_module\.ko$' "$entries" | head -n1)"
(
    cd "$extract_dir"
    gzip -dc "$INITRAMFS" |
