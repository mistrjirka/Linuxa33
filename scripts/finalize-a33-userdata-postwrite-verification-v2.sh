...
require_in_root() {
    ...
    if [ ! -L "$full" ]; then
        echo "missing_path=$path"
        exit 21
    fi
    link="$(readlink "$full")"
    case "$link" in
        /*)
            rooted_target="$mountpoint$link"
            ;;
        *)
            parent="${path%/*}"
            [ "$parent" = "$path" ] && parent=/
            rooted_target="$mountpoint$parent/$link"
            ;;
    esac
    if [ ! -e "$rooted_target" ] && [ ! -L "$rooted_target" ]; then
        echo "missing_path_symlink_target=$path link=$link"
        exit 22
    fi
    echo "path_symlink_ok=$path->$link"
}
...
verify_absolute_root_link() {
    ...
    [ -e "$mountpoint$expected" ] || [ -L "$mountpoint$expected" ] || {
...
echo "fstab_end"
grep -Fqx "root_uuid=$expected_uuid" "$mountpoint/etc/a33x-rootfs-target" || exit 26
grep -Fqx 'target=android-userdata' "$mountpoint/etc/a33x-rootfs-target" || exit 27
...
FINISHED="$(date -Ins)"
{
    ...
    echo "rootfs_symlink_verification=passed"
...
echo
echo "A33 userdata rootfs postwrite verification v2 passed."
