...
common = load(
    "a33_u0l_collector_common",
    HERE / "flash-a33-u0i-python-direct-root-v2.py",
)
u0l = load(
    "a33_u0l_collector_flash",
...
OUTPUT_PREFIX = "u0l-openrc-cgroup-isolation-result"
FOCUS_PATTERN = re.compile(
    r"a33x-u0[kl]-|openrc|cgroup(?:\.procs)?|freqboost|\bems\b|"
    ...
    r"ext4-fs|dwc3|gadget",
    re.IGNORECASE,
)
COUNT_PATTERNS = {
    "u0l_mask_begin_count": r"a33x-u0l-openrc-cgroup-isolation: stage=mask-begin",
    "u0l_mask_success_count": r"a33x-u0l-openrc-cgroup-isolation: stage=mask-success",
    "u0l_mask_error_count": r"a33x-u0l-openrc-cgroup-isolation: error=",
    "u0k_mount_success_count": r"a33x-u0k-direct-mount: stage=mount-root-success",
    "u0k_cleanup_done_count": r"a33x-u0k-direct-mount: stage=cleanup-hooks-done",
    "u0k_switch_root_begin_count": r"a33x-u0k-direct-mount: stage=switch-root-begin",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Collect U0l previous-boot evidence after exact TWRP has been restored "
            "and booted directly"
        )
    )
    ...
    local = validate_flash_report(common, u0l.validate_local, PROFILE, root, repo)
    ...
    for source in (
        ...
        root / "build/a33-u0l-flash-preflight-audit-console.txt",
    ):
    ...
    summary_pairs: list[tuple[str, object]] = [
        ...
        ("operation", "collect-u0l-previous-boot-python"),
...
if __name__ == "__main__":
    try:
        ...
        print(f"U0l PREVIOUS-BOOT COLLECTION FAILED: {exc}", file=sys.stderr)
        raise SystemExit(1)
