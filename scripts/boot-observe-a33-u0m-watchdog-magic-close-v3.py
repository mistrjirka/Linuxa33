...
common = load(
    "a33_u0m_v3_observer_common",
    HERE / "flash-a33-u0i-python-direct-root-v2.py",
)
flash = load(
    "a33_u0m_v3_observer_flash",
    HERE / "flash-a33-u0m-watchdog-magic-close-v3.py",
)
...
PROFILE = ObserverProfile(
    ...
    output_prefix="u0m-v3-watchdog-magic-close-observation",
    observation_operation="observe-u0m-v3-watchdog-magic-close",
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Controlled observer for U0m v3 watchdog handoff boot"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"REFUSING U0m v3 OBSERVER: {exc}", file=sys.stderr)
        raise SystemExit(1)
