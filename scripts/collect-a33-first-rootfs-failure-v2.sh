...
for command in "$ADB" bash find sort awk grep tar sha256sum date python3 stat cp; do
...
"$ADB" exec-out sh -c "dd if='$TARGET' bs=2048 count=1 2>/dev/null" |
    python3 - "$DEPLOYED_IMAGE_BYTES" > "$OUT/ext4-superblock-state-v2.txt" <<'PY'
...
u16 = lambda off: struct.unpack_from("<H", sb, off)[0]
u32 = lambda off: struct.unpack_from("<I", sb, off)[0]
log_block_size = u32(0x18)
block_size = 1024 << log_block_size
blocks_lo = u32(0x04)
feature_incompat = u32(0x60)
blocks_hi = u32(0x150) if feature_incompat & 0x80 else 0
blocks = blocks_lo | (blocks_hi << 32)
filesystem_bytes = blocks * block_size
mtime = u32(0x2c)
wtime = u32(0x30)
mount_count = u16(0x34)
state = u16(0x3a)
...
print(f"mount_count={mount_count}")
print(f"filesystem_state=0x{state:04x}")
...
"$ADB" shell sh -s -- "$TARGET" > "$OUT/rootfs-openrc-ssh-state-v2.txt" <<'SH'
set -u
target="$1"
mountpoint=/tmp/a33x-first-rootfs-v2-check
mounted_here=no
...
if [ -f "$mountpoint/etc/os-release" ]; then
    cat "$mountpoint/etc/os-release" 2>&1 || true
elif [ -L "$mountpoint/etc/os-release" ]; then
    link="$(readlink "$mountpoint/etc/os-release" 2>/dev/null || true)"
    case "$link" in
        /*) cat "$mountpoint$link" 2>&1 || true ;;
        *) cat "$mountpoint/etc/$link" 2>&1 || true ;;
    esac
fi
echo "os_release_end"
...
if [[ -f "$SANITIZED" ]]; then
    ...
    grep -aEin "$PATTERN" "$SANITIZED" > "$OUT/first-rootfs-focused-last-kmsg-v2.txt" || true
else
    echo "last_kmsg_sanitized_missing=yes" > "$OUT/first-rootfs-focused-last-kmsg-v2.txt"
fi

SUPERBLOCK="$OUT/ext4-superblock-state-v2.txt"
ROOT_STATE="$OUT/rootfs-openrc-ssh-state-v2.txt"
FOCUSED="$OUT/first-rootfs-focused-last-kmsg-v2.txt"
...
HOST_KEY_COUNT="$(grep -c '^host_key path=' "$ROOT_STATE" 2>/dev/null || true)"
{
    ...
    echo "operation=collect-first-rootfs-ssh-failure-v2"
...
} | tee "$OUT/first-rootfs-failure-v2-summary.txt"

# Recreate the archive after adding the v2 evidence.
ARCHIVE="$OUT.tar.gz"
tar -C "$RESULT_ROOT" -czf "$ARCHIVE" "$(basename "$OUT")"
sha256sum "$ARCHIVE" | tee "$ARCHIVE.sha256"

echo
echo "First-rootfs SSH failure evidence v2 collected."
echo "Directory: $OUT"
echo "Archive:   $ARCHIVE"
echo "Summary:"
cat "$OUT/first-rootfs-failure-v2-summary.txt"
