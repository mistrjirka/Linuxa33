...
FLASH_PATH = HERE / "flash-a33-u0i-python-direct-root-v2.py"
spec = importlib.util.spec_from_file_location("a33_u0i_flash", FLASH_PATH)
if spec is None or spec.loader is None:
    raise SystemExit(f"cannot load flash validation module: {FLASH_PATH}")
...
USB_ID = "04e8:6860"
EXPECTED_OPERATION = "flash-exact-u0i-python-direct-root-v2"
...
def validate_flash_report(root: Path, repo: Path) -> dict[str, object]:
    local = flash.validate_local(root, repo)
    manifest_path = Path(local["manifest_path"])
    flash_report_path = root / "build/a33-first-rootfs-u0i-python-direct-root-v2-flash.txt"
    if not flash_report_path.is_file():
        refuse(f"missing U0i v2 flash report: {flash_report_path}")
    report = flash.kv(flash_report_path)
    ...
    if report_manifest.resolve() != manifest_path.resolve():
        refuse("flash report references a different U0i manifest")
    if flash.sha_file(manifest_path) != report.get("candidate_manifest_sha256"):
        refuse("U0i manifest changed after flashing")
    deploy_path = Path(report.get("deployment_report", ""))
    if not deploy_path.is_file() or flash.sha_file(deploy_path) != report.get("deployment_report_sha256"):
        refuse("deployment report changed after flashing")
    local.update(
        {
            "flash_report_path": flash_report_path,
            "flash_report": report,
...
def validate_preboot(adb: str, serial: str, local: dict[str, object]) -> dict[str, object]:
    values, sections = flash.live_state(adb, serial)
    expected_sha = str(local["expected_recovery_sha"])
    expected = {
        "recovery_sha": expected_sha,
        "userdata_resolved": flash.EXPECTED_USERDATA,
        "userdata_bytes": str(flash.EXPECTED_USERDATA_BYTES),
        "userdata_readonly": "0",
    }
    mismatches = [
    ...
    root_uuid, root_label = flash.ext4_identity(adb, serial)
    ...
    if mismatches:
        refuse("preboot TWRP/rootfs state is unsafe:\n" + "\n".join(mismatches))
    return {
        **values,
        "root_uuid": root_uuid,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Controlled Python observer for U0i v2 first-rootfs boot")
    ...
    if not 1 <= args.max_seconds <= 900:
        refuse("--max-seconds must be between 1 and 900")
    ...
    local = validate_flash_report(root, repo)
    ...
    serial = flash.select_recovery(adb, 30)
    preboot = validate_preboot(adb, serial, local)
    print(f"adb_serial={serial}")
    print("preboot_state=passed")

    result_root = root / "build/runtime-results"
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out = result_root / f"u0i-python-direct-root-v2-observation-{timestamp}"
    ...
    write_key_values(
        ...
        [
            ("created", now_iso()),
            ("operation", "observe-u0i-python-direct-root-v2"),
            ("implementation_language", "python3"),
            ("phone_ip", PHONE_IP),
            ("host_cidr", HOST_CIDR),
            ("max_seconds", args.max_seconds),
            ("flash_report", local["flash_report_path"]),
            ("flash_report_sha256", flash.sha_file(Path(local["flash_report_path"]))),
            ("expected_recovery_sha256", local["expected_recovery_sha"]),
            ("phone_partition_writes", "no"),
            ("reboot_target", "recovery"),
        ],
    )
    (out / "preboot-state.json").write_text(json.dumps(preboot, indent=2, sort_keys=True) + "\n")
    (out / "host-baseline.txt").write_text(host_snapshot(ip_cmd, lsusb_cmd), encoding="utf-8")

    start_wall = datetime.now().astimezone()
    start_mono = time.monotonic()
    flash.run([adb, "-s", serial, "reboot", "recovery"])
    ...
    observations_path = out / "observation.jsonl"
    ...
    (out / "host-final.txt").write_text(host_snapshot(ip_cmd, lsusb_cmd), encoding="utf-8")
    ...
    if journalctl:
        ...
        (out / "host-kernel-journal.txt").write_text(journal.stdout + journal.stderr, encoding="utf-8")
    ...
    summary = [
        ...
        ("host_usb_network_interface_ever", "yes" if ever["host_usb_network_interface"] else "no"),
    ...
    archive_sha = flash.sha_file(archive)
    archive.with_suffix(archive.suffix + ".sha256").write_text(f"{archive_sha}  {archive}\n", encoding="utf-8")
